...
{
    /// <summary>Ce qu'on sait d'un compte saute, sans avoir rien demande au reseau.</summary>
    ...
    sealed class RunOutcome
    {
        ...
        public int Skipped;
        public string Error;          // configuration ou reseau : rien n'a pu tourner
    ...
    {
        /// <summary>
        /// Execute le check-in pour tous les comptes configures.
        /// </summary>
        /// <param name="force">Ignore le cache journalier (bouton « Lancer maintenant »).</param>
        /// <param name="atStartup">Applique la pause et attend le reseau.</param>
        public static RunOutcome Run(Config cfg, bool force, bool atStartup)
        {
            ...
            // Le cache journalier n'existe que pour eviter de reposter le meme
            // embed. Si l'utilisateur demande justement l'embed « meme si le
            // check-in etait deja fait », il n'a plus lieu d'etre : on refait le
            // tour complet, ce qui donne des donnees fraiches plutot qu'un
            // rappel fige.
            ...
            foreach (var i in pending)
            {
                ...
                // Un echec ne marque pas la journee : on retentera au prochain
                // demarrage, ce qui rattrape une coupure reseau passagere.
                ...
                // On ne re-signale pas le meme echec a chaque redemarrage : un
                // cookie expire ne doit pas produire cinq pings dans la journee.
                ...
                if (!silent)
                {
                    ...
                    {
                        // Un webhook mort ne doit pas invalider un check-in reussi.
        ...
        static CheckInResult RunOne(Config cfg, string cookie)
        {
            ...
            // Les notes ne valent que si le compte repond : inutile de solliciter
            // Chronicle avec un cookie deja rejete.
        ...
        /// <summary>Une seule ligne, qui dit l'essentiel.</summary>
        ...
        /// <summary>
        /// Au demarrage de Windows, la carte reseau n'est pas toujours prete.
        /// On attend une resolution DNS reelle plutot qu'un delai fixe : quand
        /// le reseau est deja la, cela ne coute rien.
        /// </summary>
